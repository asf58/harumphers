...
for (const width of [320, 390, 768]) {
  for (const textScale of [1, 2]) {
    test(`grid names wrap only between words at ${width}px and ${textScale * 100}% text`, async ({ page }) => {
      ...
      for (const label of await page.locator('.grid-name-word').all()) {
      ...
      const columns = width === 320 || textScale === 2 ? 2 : 3;
      expect(await page.locator('#content').evaluate(el => getComputedStyle(el).gridTemplateColumns.split(' ').length)).toBe(columns);
      expect(await name.evaluate(el => el.clientHeight > Math.ceil(parseFloat(getComputedStyle(el).lineHeight)))).toBeTruthy();
