...
- Events: `EVENT NAME`, `DATE` (date), `SPEAKER`, `TIME`, `LOCATION` (venue/address, optional), `ROOM`, `SPEAKER PHOTO` (attachments), `NOTES` (long text), `Status` (Suggested, Upcoming, Scheduled, Completed, Cancelled), `RSVP FIELD`, `GUEST FIELD`, `CREATION KEY`, and `SETUP STATE`.
